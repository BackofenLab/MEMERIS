/*
 * $Id: discretize.c,v 1.2.4.1 2006/01/24 20:44:08 nadya Exp $
 * 
 * $Log: discretize.c,v $
 ...
 *
 * Revision 1.1.1.1  2005/07/29 00:18:18  nadya
...
)
{
  ...
  for (i=0; i<n_samples; i++) {			/* sequence */
    ...
    double **szi= s->sz;			/* sz */
    for (j=0; j<lseq; j++) {
      zi[j] = szi[0][j] = szi[1][j] = 0;
    }
  }

  /* set z and sz to 1 for selected sites */
  for (i=0; i<nsites; i++) {
    ...
    double **szi= s->sz;			/* sz */
    zi[y] = 1.0;
    if (maxima[i].ic) szi[1][y] = 1.0; else szi[0][y] = 1.0;
  }
...
)
{
  ...
  */
  for (i=0; i<n_samples; i++) {			/* sequence */
    ...
    double *z = s->z;				/* z */
    double *sz1 = s->sz[0];			/* + strand */
    double *sz2 = s->sz[1];			/* - strand */
    ...
    for (j=0; j<lseq; j++) {			/* site start */
      ...
      double zj, sz1j, sz2j;			/* rounded z's */
      ...
      sz1j = sz1[j];
      sz2j = sz2[j];
      if (pal) {				/* use z (sum of sz1 and sz2)*/
        pY[jj] = INT_LOG(sw * zj);
        pYic[jj] = (sz2j > sz1j) ? '\1' : '\0';	/* choose strand */
      } else if (sz2[j] > sz1[j]) {		/* use sz2 */
        pY[jj] = INT_LOG(sw * sz2j);
        pYic[jj] = '\1'; 			/* site on - strand */
      } else {					/* use sz1 */	
        pY[jj] = INT_LOG(sw * sz1j);
        pYic[jj] = '\0'; 			/* site on + strand */
      }
...
)
{
  ...
  */
  for (i=0; i<n_maxima; i++) {
    ...
    RND(s->z[y], 11, model->maxima[i].prob);
  }
...
#undef printf
  ...
  for (i=0; i<n_maxima; i++) {
    printf("max %d %15.12f x %d y %d\n", i, model->maxima[i].prob,
      model->maxima[i].x, model->maxima[i].y);
  }
  fflush(stdout);
#ifdef PARALLEL
