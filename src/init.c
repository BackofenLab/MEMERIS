/*
 * $Id: init.c,v 1.2 2005/10/25 19:06:39 nadya Exp $
 * 
 * $Log: init.c,v $
...
)
{
  ...
  DO_STANDARD_COMMAND_LINE(1,
    USAGE(USAGE:\n\tmemeris\t<dataset> [optional arguments]\n);
  ...
  /* added by M.H. */
  /* check if both secondary structure information and revcomp option is used; if so set invcomp = FALSE
     since it makes no sense to use the reverse complementary strand for RNAs 
	  check if a fixed motif length is given   
	  check if a dna is set */
