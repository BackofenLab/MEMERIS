/*
 * $Id: pssm-distr.c,v 1.2 2005/10/25 21:23:44 nadya Exp $
 * 
 * $Log: pssm-distr.c,v $
...
)
{
  ...
  pdf = calc_pssm_pdf(w, alen, range, pssm, prob);
  if (!pdf) return NULL;
...
)
{
  ...
  for (i=0; i < w; i++) {		/* loop over columns in motif */
    ...
    swap(pdf_new, pdf_old, double *);	/* new column: swap old and new pdfs */
	...
	if (pdf_old[k] != 0) pdf_new[k + score] += pdf_old[k] * prob[j];
	if (k+score >= size) {
	  fprintf(stderr, 
