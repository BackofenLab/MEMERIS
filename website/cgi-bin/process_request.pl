#!@WHICHPERL@
##
## $Id: process_request.pl,v 1.6 2005/12/01 01:18:53 tbailey Exp $
##
...
$jaspar_root = "http://jaspar.cgb.ki.se";
$jaspar_url = "$jaspar_root/cgi-bin/jaspar_db.pl";
#
...
sub mast_search {
  ...
  while (<MASTFORM>) {
    if (/INPUT NAME=\"motifs\"/) {		# remove this line
...
sub submit_pspm_to_jaspar {
  ...
  print_header("Search JASPAR with motif $number");
  ...
  my $req = POST $jaspar_url,
    ...
    Content => [ 'matrix_string' => $pspm,  'rm' => 'compare'];
  my $request = $ua->request($req);
  $content = $request->content;

  # put in the absolute URLs : this is FRAGILE!
  $content =~ s#/TEMP#$jaspar_root/TEMP#g;
